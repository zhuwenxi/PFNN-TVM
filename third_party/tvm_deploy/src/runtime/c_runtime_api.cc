...
std::string NormalizeError(std::string err_msg) {
  ...
  if (stack_trace.size() != 0 || file_name.length() != 0) {
    ...
    for (const auto& line_ : stack_trace) {
      // Heuristic to detect python ffi.
      if (line_.find("libffi.so") != std::string::npos ||
          line_.find("core.cpython") != std::string::npos) {
        ffi_boundary = true;
      }
      ...
      if (line_.find("(TVMFuncCall") != std::string::npos) {
        ffi_boundary = true;
      }
    }
  }
