...
from utils.tune_task import android_autotvm_tune_task

dtype = "float32"

@autotvm.template('cubic_matmul_tvm_xsmm_interpA')
def gemm_cubic_interpA_kernel_fuseK_xsmm_template(dim, name, target):
    M = dim[0]
    N = dim[1]
    K = dim[2]
    global cfg
    ...
    # with xsmm
    cfg.define_reorder('reorder', [xt, xo, kt, yt, yo, ko], 'all')
    cfg.define_annotate('annotate', [xt, xo, kt, yt, yo, ko], 'try_unroll_vec')
    cfg['reorder'].apply(s, C, [xt, xo, kt, yt, yo, ko])
    cfg['annotate'].apply(s, C, [xt, xo, kt, yt, yo, ko])
...
def autotune_interp_A_with_xsmm_kernel(dim, target, name='defaultAutotune', use_tune=False, print_lower=True):
    log_dir = './logs/logs_interpA_autoTVM_xsmm/tune_M{}_N{}_K{}.log'.format(dim[0], dim[1], dim[2])

    if use_tune:
        task = autotvm.task.create('cubic_matmul_tvm_xsmm_interpA', args=(dim, name, target), target=target)
    ...
    with autotvm.apply_history_best(log_dir):
        with tvm.target.Target(target):
            s, args = gemm_cubic_interpA_kernel_fuseK_xsmm_template(dim, name, target)
            cubic_gemm_func = tvm.build(s, args, name=name)
            nc = cfg['tile_y'].size[-1]
            kc = cfg['tile_k'].size[-1]
            if print_lower:
                print("========= autotune interpA with xsmm ===========")
